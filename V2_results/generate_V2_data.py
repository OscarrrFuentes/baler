...
for path, letter in zip(samples, ["A", "B", "C", "D"]):
    ...
    with uproot.open(path + ":mini") as t:
        ...
        for data in tree.iterate(
            ...
            tight_ID_mask = np.all(data.photon_isTightID.to_numpy(), axis=1)
            data = data[tight_ID_mask]
...
np.savez(save_path+"V2_precut_original.npz", data=full_data, names=names)
np.savez(save_path+"V2_precut_normalised.npz", data=full_norm_data, names=names)
np.savez(save_path+"V2_precut_scales.npz", data=scales, names=names)
np.savez(save_path+"V2_photon_etcone20.npz", data=etcones, names=np.array(["etcone1", "etcone2"]))
