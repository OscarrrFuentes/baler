...
def load_data(args):
    with (np.load(BALER_PATH+"workspaces/higgs/data/V2_precut_normalised.npz") as orig_file,
          np.load(BALER_PATH+f"V2_results/V{args.v}/decompressed.npz") as decomp_file,
          np.load(BALER_PATH+"workspaces/higgs/data/V2_precut_scales.npz") as scale_file):
...
def apply_cuts(precut_data):
    ...
    etcone_path = "/Users/oscarfuentes/masters_project/baler_sem1/workspaces/higgs/data/photon_etcones.npz"
    with np.load(etcone_path) as etcone_file:
        etcone_cuts = cut_isolation_et(etcone_file["data"])
    return pt_cuts & eta_cuts & etcone_cuts
...
def main():
    ...
    plot_both_data(orig_masses, decomp_masses, f"V2_results/V{args.v}/", args)

    # print("Plotting resolutions...")
    # plot_resolutions(orig_masses, decomp_masses, "mass", hist_yscale="log", savename=f"V2_results/V{args.v}/mass_resolution")
