...
def load_data(args):
    with (np.load(BALER_PATH+"workspaces/higgs/data/V3_precut_normalised.npz") as orig_file,
          np.load(BALER_PATH+f"V3_results/V{args.v}/decompressed.npz") as decomp_file,
          np.load(BALER_PATH+"workspaces/higgs/data/V3_precut_scales.npz") as scale_file):
...
def plot_resolutions(
    ...
    # Calculate the resolution histogram
    resolutions = (original - reconstructed) / (original + reconstructed)
    res_n, res_bins = np.histogram(resolutions, bins=100)
    ...
    bounds = np.array([np.percentile(resolutions, 1), np.percentile(resolutions, 99)])
    resolution_xlims = [-np.amax(abs(bounds)), np.amax(abs(bounds))]
    cut_hist_ratios  = [x for x in resolutions if x > resolution_xlims[0] and x < resolution_xlims[1]]
    ...
    iqr = np.percentile(resolutions, 75) - np.percentile(resolutions, 25)
    ...
    ax4.text(
        ...
        np.amax(ratio_n)*0.7,
        f"Max: {np.amax(resolutions):.3f}\nMin: {np.amin(resolutions):.3f}"
        + f"\nMean: {np.mean(resolutions):.3f}\nMedian: {np.median(resolutions):.3f}"
        + f"\nIQR: {iqr:.3f}",
        fontsize=8,
    )
...
def corner_plot(args, orig_data, decomp_data, orig_masses, decomp_masses, plot_all=False):
    ...
    fig2.savefig(f"V2_results/V{args.v}/Ept_corner_plot.png")
    # plt.show()
    plt.close()
...
def main():
    ...
    plot_both_data(orig_masses, decomp_masses, f"V3_results/V{args.v}/", args)
### Change matplotlib params

    # print("Plotting resolutions...")
    # plot_resolutions(orig_masses, decomp_masses, "mass", hist_yscale="log", savename=f"V3_results/V{args.v}/mass_resolution")
