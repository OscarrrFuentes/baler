...
def load_data(args):
    with (np.load(BALER_PATH+"workspaces/higgs/data/V4_precut_normalised.npz") as orig_file,
          np.load(BALER_PATH+f"V4_results/V{args.v}/decompressed.npz") as decomp_file,
          np.load(BALER_PATH+"workspaces/higgs/data/V4_precut_scales.npz") as scale_file):
...
def corner_plot(args, orig_data, decomp_data, orig_masses, decomp_masses, plot_all=False):
    ...
    fig2.savefig(f"V5_results/V{args.v}/Ept_corner_plot.png")
    # plt.show()
    plt.close()
...
def main():
    ...
    plot_both_data(orig_masses, decomp_masses, f"V5_results/V{args.v}/", args)
### Change matplotlib params
    # print("Plotting resolutions...")
    # plot_resolutions(orig_masses, decomp_masses, "mass", hist_yscale="log", savename=f"V5_results/V{args.v}/mass_resolution")
